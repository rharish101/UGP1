...
def data_loader(FLAGS):
    with tf.device("/cpu:0"):
        ...
        Data = collections.namedtuple(
            "Data",
            "paths_LR, paths_HR, inputs, targets, image_count, "
            "steps_per_epoch",
        )
        ...
        if (not os.path.exists(FLAGS.input_dir_LR)) or (
            not os.path.exists(FLAGS.input_dir_HR)
        ):
...
def test_data_loader(FLAGS):
    ...
    Data = collections.namedtuple(
        "Data", "paths_LR, paths_HR, inputs, targets"
    )
