...
def _FSpecialGauss(size, sigma):
    ...
    if size % 2 == 0:
        ...
        stop -= 1
    x, y = tf.meshgrid(
        tf.range(offset + start, stop), tf.range(offset + start, stop)
    )
...
def _SSIMForMultiScale(
    ...
    if img1.shape.ndims != 4:
        raise RuntimeError(
            "Input images must have four dimensions, not %d", img1.shape.ndims
        )
    ...
    if filter_size:
        ...
        window = tf.cast(
            tf.concat([window] * img1.shape[-1], axis=-2), tf.float64
        )
...
def MultiScaleSSIM(
    ...
    downsample_filter = tf.concat(
        [downsample_filter] * img1.shape[-1], axis=-2
    )
    ...
    def my_convolve_pad(x, y, mode="SYMMETRIC"):
        ...
        return tf.nn.depthwise_conv2d(
            pad_x, y, strides=[1] * 4, padding="VALID"
        )
    ...
    for _ in range(levels):
        ...
        mcs.append(cs)
        filtered = [
            my_convolve_pad(im, downsample_filter, mode="REFLECT")
            for im in [im1, im2]
        ]
        im1, im2 = [x[:, ::2, ::2, :] for x in filtered]
    return tf.reduce_prod(
        tf.stack(mcs[0 : levels - 1]) ** weights[0 : levels - 1]
