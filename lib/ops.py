...
def conv2_NCHW(
    ...
    # kernel: list of 2 integer specifying the width and height of the 2D
    # convolution window
    with tf.variable_scope(scope):
        if use_bias:
            return slim.conv2d(
  ...
  """
    with tf.variable_scope(scope, "vgg_19", [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope(
            ...
            net = slim.repeat(
                inputs, 2, slim.conv2d, 64, 3, scope="conv1", reuse=reuse
            )
            net = slim.max_pool2d(net, [2, 2], scope="pool1")
            net = slim.repeat(
                net, 2, slim.conv2d, 128, 3, scope="conv2", reuse=reuse
            )
            net = slim.max_pool2d(net, [2, 2], scope="pool2")
            net = slim.repeat(
                net, 4, slim.conv2d, 256, 3, scope="conv3", reuse=reuse
            )
            net = slim.max_pool2d(net, [2, 2], scope="pool3")
            net = slim.repeat(
                net, 4, slim.conv2d, 512, 3, scope="conv4", reuse=reuse
            )
            net = slim.max_pool2d(net, [2, 2], scope="pool4")
            net = slim.repeat(
                net, 4, slim.conv2d, 512, 3, scope="conv5", reuse=reuse
            )
