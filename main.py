...
Flags.DEFINE_string(
    ...
    "SRResnet",
    "The type of pretrained model (SRGAN or SRResnet)",
)
Flags.DEFINE_boolean("is_training", True, "Training => True, Testing => False")
Flags.DEFINE_string(
    "vgg_ckpt", "./vgg19/vgg_19.ckpt", "path to checkpoint file for the vgg19"
)
...
Flags.DEFINE_string(
    "input_dir_LR", None, "The directory of the input resolution input data"
)
Flags.DEFINE_string(
    "input_dir_HR", None, "The directory of the high resolution input data"
)
Flags.DEFINE_string(
    "input_dir_SEG",
    None,
...
Flags.DEFINE_integer(
    "name_queue_capacity",
    2048,
    "The capacity of the filename queue (suggest large to ensure"
    "enough random shuffle.",
)
Flags.DEFINE_integer(
    "image_queue_capacity",
    2048,
    "The capacity of the image queue (suggest large to ensure"
    "enough random shuffle",
)
Flags.DEFINE_integer(
    "queue_thread",
    10,
    "The threads of the queue (More threads can speedup the training process.",
)
...
Flags.DEFINE_string(
    "perceptual_mode", "VGG54", "The type of feature used in perceptual loss"
)
...
Flags.DEFINE_float(
    "vgg_scaling",
    0.0061,
    "The scaling factor for the perceptual loss if using vgg perceptual loss",
)
# The training parameters
Flags.DEFINE_float(
    "learning_rate", 0.0001, "The learning rate for the network"
)
Flags.DEFINE_integer(
...
if FLAGS.mode == "test":
    ...
    targets_raw = tf.placeholder(
        tf.float32, shape=[1, None, None, 3], name="targets_raw"
    )
    ...
    with tf.name_scope("convert_image"):
        ...
        converted_inputs = tf.image.convert_image_dtype(
            inputs, dtype=tf.uint8, saturate=True
        )
    ...
    with tf.name_scope("encode_image"):
        ...
        if seg_output is not None:
            ...
            save_fetch["seg_outputs"] = (
                tf.map_fn(
                    tf.image.encode_png,
                    converted_seg_outputs,
                    dtype=tf.string,
                    name="seg_output_pngs",
                ),
            )
            save_fetch["seg_targets"] = (
                tf.map_fn(
                    tf.image.encode_png,
                    converted_seg_targets,
                    dtype=tf.string,
                    name="seg_target_pngs",
                ),
            )

    # Define the weight initiallizer (In inference time, we only need to
    # restore the weight of the generator)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter):
            ...
            if seg_output is not None:
                seg_target_im = np.array([test_data.seg_targets[i]]).astype(
                    np.float32
                )
            else:
                seg_target_im = None
            ...
            psnr_res = sess.run(
                psnr, feed_dict={inputs_raw: input_im, targets_raw: target_im}
            )
            print("PSNR: {}".format(results["psnr"]))
            # psnr_res, ssim_res = sess.run(
            # [psnr, ssim],
            # feed_dict={inputs_raw: input_im, targets_raw: target_im},
            # )
            # print(
            # "PSNR: {}, SSIM: {}".format(results["psnr"], results["ssim"])
            # )
...
# the inference mode (just perform super resolution on the input image)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter):
            ...
            results = sess.run(
                save_fetch, feed_dict={inputs_raw: input_im, path_LR: path_lr}
            )
            filesets = save_images(results, FLAGS)
            for i, f in enumerate(filesets):
                print("evaluate image", f["name"])
...
# The training mode
    ...
    if FLAGS.task == "SRGAN":
        ...
        tf.summary.scalar(
            "generator_loss",
            Net.content_loss + FLAGS.ratio * Net.adversarial_loss,
        )
    elif FLAGS.task == "MAD_SRGAN":
        tf.summary.scalar("discriminator_loss", Net.discrim_loss)
        tf.summary.scalar("adversarial_loss", Net.adversarial_loss)
    ...
    # Here if we restore the weight from the SRResnet the var_list2 do not need
    # to contain the discriminator weights. On contrary, if you initial your
    # weight from other SRGAN checkpoint, var_list2 need to contain
    ...
    if not FLAGS.perceptual_mode == "MSE":
        vgg_var_list = tf.get_collection(
    ...
    sv = tf.train.Supervisor(
        logdir=FLAGS.summary_dir, save_summaries_secs=0, saver=None
    )
    with sv.managed_session(config=config) as sess:
        if (FLAGS.checkpoint is not None) and (
            FLAGS.pre_trained_model is False
        ):
            print("Loading model from the checkpoint...")
            checkpoint = tf.train.latest_checkpoint(FLAGS.checkpoint)
            saver.restore(sess, checkpoint)

        elif (FLAGS.checkpoint is not None) and (
            FLAGS.pre_trained_model is True
        ):
            print("Loading weights from the pre-trained model")
            if FLAGS.task == "MAD_SRGAN":
        ...
        if FLAGS.max_epoch is None:
            if FLAGS.max_iter is None:
                raise ValueError(
                    "one of max_epoch or max_iter should be provided"
                )
            else:
        ...
        for step in range(max_iter):
            ...
            if ((step + 1) % FLAGS.display_freq) == 0:
                train_epoch = math.ceil(
                    results["global_step"] / data.steps_per_epoch
                )
                train_step = (
                    results["global_step"] - 1
                ) % data.steps_per_epoch + 1
